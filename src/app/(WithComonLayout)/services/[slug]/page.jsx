...
const ServiceDetails = async ({ params }) => {
  ...
  // Fetch the single service from MongoDB using the _id slug
  let s = null;
  try {
    const result = await getSingleProduct(slug);
    s = Array.isArray(result) ? result[0] ?? null : result ?? null;
  } catch (err) {
  ...
  // Serialize the _id to a plain string for use in hrefs
  const serviceId = s._id ? String(s._id) : slug;

  // Fetch similar services from the same category
  let similar = [];
  try {
    const all = await getProducts();
    similar = (Array.isArray(all) ? all : [])
      .filter((sv) => sv.category === s.category && String(sv._id) !== serviceId)
      .slice(0, 2);
  } catch {
    // silently ignore — similar services are non-critical
  }
              ...
              {s.caregiver.picture && (
                <div className="h-20 w-20 rounded-2xl overflow-hidden shrink-0">
                  <Image
                    ...
                    width={80}
                    height={80}
                    alt={s.caregiver.name ?? "Caregiver"}
      ...
      {similar.length > 0 && (
        <section className="mt-20">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {similar.map((s2) => {
              const s2Id = s2._id ? String(s2._id) : "";
              return (
                <Link
                  key={s2Id}
                  href={`/services/${s2Id}`}
